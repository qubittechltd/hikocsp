...
struct parse_options_result {
    struct option_type {
        ...
        [[nodiscard]] friend bool operator==(option_type const& lhs, std::string_view rhs) noexcept
        {
            return lhs.name == rhs;
        }

        [[nodiscard]] friend std::string to_string(option_type const& rhs) noexcept
        {
            if (not rhs.argument) {
                return rhs.name;
            } else if (rhs.name.starts_with("--")) {
                return fmt::format("{}={}", rhs.name, *rhs.argument);
            } else {
                return fmt::format("{} {}", rhs.name, *rhs.argument);
            }
        }
 ...
 * @return results.
 */
[[nodiscard]] parse_options_result
    ...
    for (; it != args.end(); ++it) {
        ...
        if (need_option_argument) {
            ...
            if (end_name == std::string::npos) {
                r.options.emplace_back(parse_options_result::option_type{.name= std::string{arg}, .argument = std::nullopt});
            } else {
                r.options.emplace_back(parse_options_result::option_type{.name=std::string{arg.substr(0, end_name)}, .argument =std::string{arg.substr(end_name + 1)}});
            }
            ...
            for (auto const c : arg.substr(1)) {
                if (need_option_argument) {
                    ...
                    r.options.emplace_back(parse_options_result::option_type{.name=fmt::format("-{}", c),.argument = std::nullopt});
