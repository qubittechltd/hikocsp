...
parse_csp_expression(It& first, It last, std::filesystem::path const& path, int& line_nr, bool is_filter)
{
        ...
        case '[':
            if (not quote) {
                if (stack_size == stack.size()) {
                    throw csp_error(fmt::format("{}:{}: Subexpression nesting is too deep.", path.string(), line_nr));
                }
    ...
    throw csp_error(fmt::format("{}:{}: Unexpected EOF parsing C++ expression", path.string(), line_nr));
}
    ...
    while (first != last) {
        ...
        while (first != last) {
            ...
            if (after == detail::parse_csp_after_text::verbatim) {
                ...
                while (true) {
                    if (first == last) {
                        throw csp_error(fmt::format("{}:{}: Incomplete placeholder found.", path.string(), line_nr));
