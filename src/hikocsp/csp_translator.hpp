    ...
    for (auto c : str) {
        ...
        default:
            if (c < 0x20 or c == 0x24 or c == 0x40 or c == 0x60 or c > 0x7e) {
                ...
                r += fmt::format("\\x{:02x}", static_cast<uint8_t>(c));
                x_escape = true;
                continue;
        ...
        if (token.kind == csp_token_type::verbatim) {
            ...
            if (not token.text.empty()) {
                ...
                if (num_lines == 0 or (num_lines == 1 and token.text.back() == '\n')) {
                    // Only one line.
                    co_yield translate_csp_yield(fmt::format("\"{}\"", encode_string_literal(token.text)), config);
                    ...
                    while (i != token.text.npos) {
                        ...
                        str += fmt::format("\"{}\"", encode_string_literal(token.text.substr(prev_i, i - prev_i)));
...
[[nodiscard]] generator<std::string>
translate_csp(const csp::generator<csp::csp_token<const char *>> & tokens, std::filesystem::path const& path, translate_csp_config const& config) noexcept
{
    using namespace std::literals;
